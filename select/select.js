const getTemplate = (data = [], placeholder, selectedId) => {
  let text = placeholder ?? 'Selected element'
  const items = data
    .map((i) => {
      let cls = ''
      if (i.id === selectedId) {
      ...
      return `<li class="select__item ${cls}" data-type="item" data-id="${i.id}">${i.value}</li>`
    })
    .join('')
...
export class Select {
  ...
  clickHandler(event) {
    ...
    else if (type === 'item') this.select(event.target.dataset.id)
